...
export function CartLink({ className = "" }: CartLinkProps) {
  ...
  return (
    <Link
      href="/cart"
      aria-label={itemCount ? `Shopping cart, ${itemCount} item${itemCount === 1 ? "" : "s"}` : "Shopping cart, empty"}
    ...
    >
      ...
      {itemCount ? <span aria-hidden="true" className="absolute -right-0.5 -top-0.5 flex size-5 items-center justify-center rounded-full bg-[#183247] text-xs font-bold text-white">{itemCount > 99 ? "99+" : itemCount}</span> : null}
    </Link>
  );
}
